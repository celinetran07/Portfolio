 /* ── FLOATING PETALS IN HERO ── */
    ...
    document.querySelectorAll('.contact-link-item').forEach(l => linkObserver.observe(l));
...
function buildFlower(cx, cy, r, count) {
  return Array.from({ length: count }, (_, i) => {
   ...
   const wave = 0.75 + 0.25 * Math.cos(angle * 5); /* cos instead of sin + higher depth */
    return {
      rx: cx + Math.cos(angle) * r * wave,
      ry: cy + Math.sin(angle) * r * wave,  /* remove the * 0.9 to make it circular */
      ...
      let points = [], mouse = { x: -9999, y: -9999 }, time = 0;
      ...
      function drawFace(pts, r) {
        ...
        ctx.beginPath();
        ctx.ellipse(c.x - s * 0.55, c.y + s * 0.05, s * 0.18, s * 0.10, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.ellipse(c.x + s * 0.55, c.y + s * 0.05, s * 0.18, s * 0.10, 0, 0, Math.PI * 2);
        ctx.fill();
      ...
      function update() {
        ...
        const bodyY = Math.sin(time * 1.1) * 10 * scale + Math.cos(time * 0.6) * 4 * scale;
      ...
      function render() {
        ...
        drawCurve(points);
        ctx.fillStyle   = CONFIG.fillColor;   ctx.fill();
        ctx.strokeStyle = CONFIG.strokeColor; ctx.lineWidth = CONFIG.strokeWidth; ctx.stroke();
